...
**Status:** living, last reconciled 2026-09-17 — protocol references moved v1.7.0 → v1.12.0, the version the client consumes as of v0.5.0; see [`v0.5-plan.md`](./v0.5-plan.md) for the work behind that. Previously reconciled 2026-09-12 (v1.4.0 → v1.7.0, audited in [`upstream-drift-2026-09-12.md`](./upstream-drift-2026-09-12.md)) and 2026-07-20 (evening). Migrated from `core-agent/docs/mast/web-design.md` on 2026-06-12 as part of mast-web repo init. Substantively revised 2026-06-13: dropped "go:embed into the agent binary" as the *default* deployment shape — the actual answer is a spectrum of four operator-selectable options (hosted SPA, container image, agent `--ui` flag, self-host tarball). Reconciled 2026-07-20: bumped protocol version references from v1.1.0 through v1.4.0 (v1.3.0 was consumed by the digest-`savings` sidecar; v1.4.0 added the capability manifest per core-agent#344 + core-tui#68), flipped Phase C★ to Shipped, redirected mast docs cross-refs to the mast repo (which now exists as docs-only pre-code-fork). Companion to [fork-design.md](https://github.com/go-steer/mast/blob/main/docs/fork-design.md) (which specifies `mast` as the lean-fork project) and [positioning.md](https://github.com/go-steer/mast/blob/main/docs/positioning.md) (the thesis). Release plans, newest first and each carrying its own executed marker: [`v0.5-plan.md`](./v0.5-plan.md), [`v0.4-plan.md`](./v0.4-plan.md), [`v0.3-plan.md`](./v0.3-plan.md), [`v0.2-catch-up-plan.md`](./v0.2-catch-up-plan.md) — the last of which this line used to name as live work, three releases ago. This doc covers `mast`'s web-based interactive surface — the operator-facing UI that replaces the embedded terminal TUI in the lean fork's scope.
...
|---|---|---|
| Language | **Vanilla JS** for v0.1 (TypeScript optional later) | Cogo-wasm2 is vanilla JS and that worked. Adding a build pipeline (TS, bundler) on day one trades simplicity for future-proofing we don't yet need. The original clause here was "revisit at v0.5 if `app.js` exceeds ~3000 LOC". It did — 4,305 — and the answer at v0.4 was to delete it rather than to compile it, which is the [`shells-design.md`](./shells-design.md) split: a shared core plus per-shell presentations, no file large enough for the question to come back. **Checkpoint discharged at v0.5.0; still vanilla JS, still no build step.** |
...
| Connection transport | **SSE for events, fetch for requests** (v0.1); evaluate WebSocket later | Mast's `pkg/attach/` publishes SSE per protocol v1.12.0 (const `ProtocolVersion` in `pkg/attach/events.go`; spec doc at `core-tui/docs/sse-event-stream-protocol.md`). v1.3.0 was consumed 2026-07-17 by the digest-`savings` sidecar; v1.4.0 (core-agent#344 + core-tui#68, both merged 2026-07-20) added the `capabilities.features` / `slash_commands` / `agent` / `caller_id` manifest fields + `/whoami` endpoint + `_render` / `_schema` slash-response conventions; v1.5.0 added the `pause` event and the pause gate, v1.6.0 an optional `title` on session rows, v1.7.0 the `wake` event (all three 2026-08-19, consumed 2026-09-12 — see [`upstream-drift-2026-09-12.md`](./upstream-drift-2026-09-12.md)). v1.8.0 through v1.12.0 were consumed 2026-09-17 ([`v0.5-plan.md`](./v0.5-plan.md)): a `canceled` kind on `turn-error` (v1.8.0), an optional `tools` grant on configured-subagent rows (v1.9.0, core-agent#768), the ACL and title routes plus approval attribution (v1.10.0, core-agent#797/#808/#830), and two **behaviour** changes rather than additions — an inject no longer releases a hold (v1.11.0, core-agent#878) and `state: "running"` / `turn_in_flight` become reachable on `GET /status` while subagent stop starts reporting what it did rather than what was asked (v1.12.0, core-agent#896/#897). Reusing the existing channel is the lowest-effort path. WebSocket bidirectional may be worth it for the operator-input path, but SSE + POST works for v0.1. |
...
**Phases A+B+C can start before the mast fork lands.** The attach protocol they consume already exists in core-agent's `pkg/attach/` (HTTP/SSE typed events per protocol v1.12.0). mast-web doesn't care which binary serves the attach endpoint — same protocol, same client. Build mast-web against `core-agent --attach-listen :7777` today; when mast forks and ships, flip the endpoint pointer. Zero code change on the mast-web side.
...
- [SSE wire protocol spec](https://github.com/go-steer/core-tui/blob/main/docs/sse-event-stream-protocol.md) — the versioned wire contract (currently v1.12.0)
